...
function setup_vendor() {
    ...
    export DEMENTED_ROOT="$3"
    if [ ! -d "$DEMENTED_ROOT" ]; then
        echo "\$DEMENTED_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$DEMENTED_ROOT/$OUTDIR" ]; then
        mkdir -p "$DEMENTED_ROOT/$OUTDIR"
    fi

    export PRODUCTMK="$DEMENTED_ROOT"/"$OUTDIR"/"$DEVICE"-vendor.mk
    export ANDROIDMK="$DEMENTED_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$DEMENTED_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$DEMENTED_ROOT"/"$OUTDIR"/proprietary
